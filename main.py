...
def clean_pdf(pdf_input):
    ...
    for i in range(last_page_n):
        ...
        if current_page.extractText()[-6] != next_page.extractText()[-6]:
            pdf_writer.addPage(current_page)
            print(f'Added page: {i}')
    # Add last page
    if pdf_reader.getPage(last_page_n-1) != pdf_reader.getPage(last_page_n):
        pdf_writer.addPage(pdf_reader.getPage(last_page_n))
